...
// Struttura pacchetto binario compatto (6 byte esatti)
...
class MyServerCallbacks: public BLEServerCallbacks {
    void onConnect(BLEServer* pServer, esp_ble_gatts_cb_param_t *param) { 
      ...
      // Richiede intervallo di connessione rapido (7.5ms - 15ms)
...
void setup() {
  ...
  Serial.println("ESP32 LiDAR (Binario + Timestamp) avviato.");
}

void loop() {
  // Parsing frame 9-byte del TF-Luna
  if (lidarSerial.available() >= 9) {
    if (lidarSerial.read() == 0x59 && lidarSerial.read() == 0x59) {
      ...
      // Scarta i restanti 5 byte del frame
      for (int i = 0; i < 5; i++) {
        lidarSerial.read();
      }

      // Invio binario con timestamp istantaneo
