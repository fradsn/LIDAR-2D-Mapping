...
class TargetTracker(QObject):
    ...
    def reset_to_full_scan(self):
        ...
        # Invia subito il comando alla base per tornare a girare a 360°
        if self.ble:
    ...
    def process_targets(self, targets):
        ...
        if self.state == TrackerState.SEARCHING:
            valid_candidates = [t for t in targets if t.distance_m > 0.25 and t.radius_m <= 0.65]

            if valid_candidates:
                primary = min(valid_candidates, key=lambda t: t.distance_m)
                self.lock_confirmations += 1

                if self.lock_confirmations >= 3:
        ...
        # -------------------------------------------------------------
        # STATO 2: LOCKED (Inseguimento a Cono Stretto)
        # -------------------------------------------------------------
        elif self.state == TrackerState.LOCKED:
            matched_target = None
            if targets:
                for t in targets:
                    az_diff = abs(t.azimuth_deg - self.target_azimuth)
                    if az_diff > 180.0:
                        az_diff = 360.0 - az_diff
                    if az_diff <= (self.cone_half_width_deg + 10.0) and abs(t.distance_m - self.target_distance) < 0.50:
                        matched_target = t
                        break
        ...
        # -------------------------------------------------------------
            ...
            if matched_target:
                ...
                if (now - self.last_seen_time) > 4.0:
                    self.reset_to_full_scan()

    def _update_cone(self, now_time, force=False):
        if self.target_distance < 1.2:
            self.cone_half_width_deg = 20.0
        elif self.target_distance < 2.5:
            self.cone_half_width_deg = 16.0
        else:
            self.cone_half_width_deg = 13.0

        min_deg = (self.target_azimuth - self.cone_half_width_deg) % 360.0
        max_deg = (self.target_azimuth + self.cone_half_width_deg) % 360.0

        self.tracking_sector_updated.emit(min_deg, max_deg, self.target_azimuth)

        if force or (now_time - self._last_cmd_time > 0.35):
            d_min = abs(min_deg - self._last_sent_sector[0])
            d_max = abs(max_deg - self._last_sent_sector[1])
            if force or d_min > 2.5 or d_max > 2.5:
    ...
    def _widen_cone(self, now_time):
        w_half = 30.0
        min_deg = (self.target_azimuth - w_half) % 360.0
        max_deg = (self.target_azimuth + w_half) % 360.0
